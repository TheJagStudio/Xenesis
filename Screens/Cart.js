...
function Cart({ navigation }) {
	...
	if (status === "loading") {
		departmentFetcher();
		return <StyledLinearGradient colors={colorScheme === "dark" ? ["#351f62", "#221144", "#221144"] : ["#FFFFFF", "#FFFFFF", "#e3c9ff"]} className="bg-white dark:bg-[#221144] h-full" style={{ paddingTop: 40 }}></StyledLinearGradient>;
	}
	return (
			...
			<ScrollView showsHorizontalScrollIndicator={false} className="overflow-visible" stickyHeaderHiddenOnScroll={false} stickyHeaderIndices={[0]}>
				{event.map((item, index) => (
					<StyledTouchableOpacity
						key={index}
						onPress={() => {
							navigation.navigate("EventDetails", { event: item[0] });
						}}
					>
