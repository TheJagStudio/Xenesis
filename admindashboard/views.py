    ...
    if request.user.is_authenticated:
        if request.user.is_superuser:
                ...
                if event.name not in eventArr:
                    try:
                        if event.isTeamEvent == True:
                            eventArr[event.name] = [1, int((event.teamPrice).split("-")[0]),event.name, event.posterImage]
                        else:
                            eventArr[event.name] = [1, int((event.price).split("-")[0]),event.name, event.posterImage]
                    except:
                        eventArr[event.name] = [1, int(event.price),event.name, event.posterImage]
                else:
            ...
            for event in eventArr.keys():
                ...
                eventArr[event] = [eventArr[event][0], eventArr[event][1]*eventArr[event][0], eventArr[event][2], eventArr[event][3],eventpercentage]
